...
export function ChatWindowScrollToBottomSave() {
  ...
  const setScrollChatToBottom = useChatStore(
    (state) => state.setScrollChatToBottom,
  );
...
export function Chat() {
  ...
  const isAnalysisRunning = useChatStore(
    (state) => !!state.currentAnalysisRequestId,
  );
  ...
  const isLastMessageFromUser =
    ...
    chat.message_groups[chat.message_groups.length - 1].agent_id === 'user';
